...
export const AuvoraReportSchema = z.object({
  ...
  evidence_gaps: z.array(EvidenceGapSchema).max(5),
  blind_spots: z.array(BlindSpotSchema).max(5),
  stakeholders: z.array(StakeholderSchema).max(5),
  risks: z.array(RiskSchema).max(6),
  consequences: z.array(ConsequenceChainSchema).max(4),
