interface BlindSpotResult {
  blind_spot_title: string;
  result: "surfaced" | "not_observed" | "inconclusive" | string;
  explanation: string;
}

interface BlindSpotAuditProps {
  blindSpotResults: BlindSpotResult[];
}

const BADGE_MAP: Record<string, { label: string; style: string }> = {
  surfaced: {
    label: "Surfaced",
    style: "bg-rose-500/10 text-rose-400 border-rose-500/20",
  },
  not_observed: {
    label: "Not Observed",
    style: "bg-zinc-800 text-zinc-300 border-zinc-700",
  },
  inconclusive: {
    label: "Inconclusive",
    style: "bg-amber-500/10 text-amber-400 border-amber-500/20",
  },
};

export function BlindSpotAudit({ blindSpotResults }: BlindSpotAuditProps) {
  if (!blindSpotResults || blindSpotResults.length === 0) {
    return null;
  }

  return (
    <div className="space-y-4 pt-4 border-t border-zinc-800">
      <div className="space-y-1">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-zinc-300">
          Blind Spot Audit
        </h3>
        <p className="text-base text-zinc-400">
          Did the blind spots Auvora identified appear in reality?
        </p>
      </div>

      <div className="divide-y divide-zinc-800/60 rounded-xl border border-zinc-800 bg-zinc-950/60">
        {blindSpotResults.map((item, idx) => {
          const badge = BADGE_MAP[item.result] || BADGE_MAP["inconclusive"]!;

          return (
            <div key={idx} className="p-4 space-y-2">
              <div className="flex items-start justify-between gap-3">
                <span className="text-sm font-semibold text-zinc-200 leading-snug">
                  {item.blind_spot_title}
                </span>
                <span
                  className={`shrink-0 rounded-full border px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${badge.style}`}
                >
                  {badge.label}
                </span>
              </div>
              <p className="text-base text-zinc-400 leading-relaxed">{item.explanation}</p>
            </div>
          );
        })}
      </div>
    </div>
  );
}
