...
function updateFile(filePath) {
  ...
  // Cancel / Secondary Button (bg-zinc-900 text-zinc-300)
  const oldSecBtn = 'hover:bg-zinc-800';
  const newSecBtn = 'transition-all duration-200 hover:bg-zinc-800 hover:text-white focus:outline-none focus:ring-2 focus:ring-zinc-500 focus:ring-offset-2 focus:ring-offset-zinc-950 active:scale-95 motion-reduce:transition-none motion-reduce:transform-none';
  // Be careful with hover:bg-zinc-800, we only want to replace it on buttons/links. Let's do a targeted replace for the specific classes found.
  
  const oldCancelBtn1 = 'className="rounded-md border border-zinc-800 bg-zinc-900 px-3 py-1.5 text-sm text-zinc-300 hover:bg-zinc-800"';
  const newCancelBtn1 = 'className="rounded-md border border-zinc-700 bg-zinc-900 px-3 py-1.5 text-sm font-semibold text-zinc-300 transition-all duration-200 hover:bg-zinc-800 hover:text-white focus:outline-none focus:ring-2 focus:ring-zinc-500 focus:ring-offset-2 focus:ring-offset-zinc-950 active:scale-95 motion-reduce:transition-none motion-reduce:transform-none"';
  content = content.replace(new RegExp(oldCancelBtn1, 'g'), newCancelBtn1);
...
updateFile('src/app/decisions/new/page.tsx');
console.log('Updated src/app/decisions/new/page.tsx');
